...
func (s *AppState) ReloadSkills() skills.Discovery {
	...
	s.skills = skills.LoadMetadata(skills.Discover(configDirs))
	return s.GetSkills()
}
...
func (s *AppState) ReloadSubagents() subagents.Discovery {
	...
	s.subagents = subagents.LoadMetadata(subagents.Discover(configDirs))
	return s.GetSubagents()
}
...
func (s *AppState) GetContextBreakdown() llm.ContextBreakdown {
	systemPrompt := llm.Build(s.workingDir, s.SkillsCatalog(), s.SubagentsCatalog(), s.Mode(), s.agentCfg)
