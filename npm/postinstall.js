#!/usr/bin/env node
// Downloads the keen-agent binary for the current platform during npm install.
...
const REPO = "mochow13/keen-agent";
...
const binaryName = platform === "windows" ? "keen-agent.exe" : "keen-agent";
...
function download(url) {
  return new Promise((resolve, reject) => {
    const chunks = [];
    const get = (u) =>
      https.get(u, { headers: { "User-Agent": "keen-agent-npm-installer" } }, (res) => {
...
async function main() {
  console.log(`[keen-agent] Downloading keen-agent v${VERSION} for ${platform}/${arch}...`);
  ...
  console.log("[keen-agent] Checksum verified.");

  extractBinary(archiveBuf);
  console.log(`[keen-agent] Installed to ${binaryPath}`);
}
